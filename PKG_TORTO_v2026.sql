...
IS
   ...
   FUNCTION fc_torto_sol_lg (plg     IN VARCHAR2,
                             ps11    IN VARCHAR2,
		                     ps12    IN VARCHAR2,
		                     ps13    IN VARCHAR2,
		                     ps14    IN VARCHAR2,
...
AS
   /*
   VERS�O:
   1.00 - Cria��o da package - 03/03/2017
   */
   ...
   BEGIN
      ...
      FOR CR
         IN (  SELECT palavra
                ...
                WHERE palavra IS NOT NULL
                  AND lg = plg
                  AND REGEXP_LIKE(palavra, '^['||ps11||ps12||ps13||ps14||ps15||ps16||ps21||ps22||ps23||ps24||ps25||ps26||ps31||ps32||ps33||ps34||ps35||ps36||']+$')
             ORDER BY LENGTH (palavra),
                      LENGTH (
                         REPLACE (
                            REPLACE (
                               REPLACE (REPLACE (palavra, ps22, ''),
                                        ps23,
                                        ''),
                               ps24,
                               ''),
